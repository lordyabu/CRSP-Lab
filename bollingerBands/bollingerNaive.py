...
class BollingerNaive(StockAlgorithmDaily):
    ...
    def __init__(self, stock_name, band_data_name='Default',identifier=-1, time_period='Daily', reset_indexes=False, step=0):
        ...
        super().__init__(stock_name = stock_name, folder_name='bollingerData',reset_indexes = reset_indexes, step = step)
    ...
    def start_position(self, action):
        if action == 'long':
            self.in_trade = True
            self.enter_trade_date = self.df.iloc[self.step]["Day"]
            self.exit_trade_date = None
    ...
    def exit_position(self):
        self.exit_trade_date = self.df.iloc[self.step]["Day"]
